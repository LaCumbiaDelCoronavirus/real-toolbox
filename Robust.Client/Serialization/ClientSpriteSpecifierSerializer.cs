...
public sealed class ClientSpriteSpecifierSerializer : SpriteSpecifierSerializer
{
            ...
            ISerializationContext? context)
    {
        ...
        var resourceCache = dependencies.Resolve<IResourceCache>();

        if (!resourceCache.ResolvePath(TextureRootName, valuePathNode.Value, out var rsiPath))
            return new ErrorNode(node, "Failed to resolve RSI path, likely missing");

        if (!resourceCache.TryGetResource(rsiPath.Value.CanonPath, out RSIResource? resource))
